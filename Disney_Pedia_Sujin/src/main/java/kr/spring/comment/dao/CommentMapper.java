...
public interface CommentMapper {
	...
	// 코멘트 수정
	@Update("UPDATE dcomment SET content=#{content}, modify_date=CURRENT_TIMESTAMP()  WHERE contents_num=#{contents_num} AND contents_type=#{contents_type} AND mem_num=#{mem_num}")
	...
	// 코멘트 좋아요
	@Insert("INSERT INTO dcomment_like (commentlike_num, comment_num, mem_num) VALUES (dcomment_like_seq_NEXTVAL(),#{comment_num},#{mem_num})")
	...
	// 코멘트 댓글 작성
	@Insert("INSERT INTO dcomment_reply (reply_num, comment_num, mem_num, content) VALUES (dcomment_reply_seq_NEXTVAL(), #{comment_num}, #{mem_num}, #{content})")
	public void insertReply(CommentReplyVO reply);

	// 코멘트 댓글 수정
	@Update("UPDATE dcomment_reply SET content=#{content}, modify_date=CURRENT_TIMESTAMP() WHERE reply_num=#{reply_num}")
